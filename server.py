...
class server:
    ...
    def vc_handler(self, message_type, message_view, message_world, message_content, message_lock):
        if message_type.startswith("vc-value-gather") and message_world == self._current_leader:
            # if this message has been seen in this view
            self.add_to_value_gather(message_content, message_lock, message_view)
            self._vc_value_counter += 1
        elif message_type.startswith("vc-blame") and message_world == self._current_leader:
            self.add_to_value_gather(message_content, message_lock, message_view)
            self._vc_blame_gather += 1
        # if someone didn't heard from the leader it's enough for me to go another round iff i got N-F messages (that's the best i can)
        if self._vc_blame_gather + self._vc_value_counter >= N - F and self._vc_blame_gather > 0:
            # VIEW CHANGE,RESTART MY WORLD
            for key, value in self._vc_value_gather.items():
                self.logit('saw lock ' + key[2] + '. mine is ' + str(self._locks[self._id]))
                #  key[2]=lock  >  my lock in my world=my_locks in my world
                if int(key[2]) > self._locks[self._id]:
                    self._locks[self._id] = int(key[2])
                    self._values[self._id] = key[1]
                    self.logit(
                        'saw higher or equal lock ' + key[2] + '. thus set my value to ' + str(self._values[self._id]))
    ...
    def add_to_value_gather(self, message_content, message_lock, message_view):
        if (message_view, message_content, message_lock) in self._vc_value_gather.keys():
            self._vc_value_gather[(message_view, message_content, message_lock)] += 1
        else:
            # else add this message to the vc gather bag
            self._vc_value_gather[(message_view, message_content, message_lock)] = 1
